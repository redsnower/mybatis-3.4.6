...
public final class TypeHandlerRegistry {
  ...
  //  记录JdbcType与TypeHandler 之间的对应关系，其中JdbcType 是枚举类型，定义对应的JDBC类型
  // 该集合主要用于从结果集中读取数据时，将JDBC类型转换为Java类型
  private final Map<JdbcType, TypeHandler<?>> JDBC_TYPE_HANDLER_MAP = new EnumMap<JdbcType, TypeHandler<?>>(JdbcType.class);
  /**
   * 记录了Java 类型向指定JdbcType 转换时，需要使用的TypeHandler 对象。例如： Java 类型中的String 可能
  ...
  private <T> TypeHandler<T> getTypeHandler(Type type, JdbcType jdbcType) {
    ...
    // 查找或者初始化Java类型对应的TypeHandler 集合
    ...
    if (jdbcHandlerMap != null) {
      ...
      if (handler == null) {
        // #591  如果jdbcHandlerMap 只注册了一个TypeHandler，则使用此TypeHandler对象
  ...
  /**
   * 首先检测TYPE_HANDLER_MAP 中指定Java类型对应的TypeHandler是否已经初始化，
   * 如果未初始化，则尝试以改Java类型的、已初始化的父类对应的TypeHandler几个初始化；如果不存在已初始化的父类，则将起对应的TypeHandler集合初始化为NULL_TYPE_HANDLER_MAP 标识
   * @param type
   * @return
   */
  ...
  // scan
  // 扫描这个包下的TypeHandler 接口实现类

  public void register(String packageName) {
    ResolverUtil<Class<?>> resolverUtil = new ResolverUtil<Class<?>>();
    // 查找指定包下面TypeHandler接口的实现类
