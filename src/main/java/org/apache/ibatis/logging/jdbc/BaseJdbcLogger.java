 ...
 *
 * 通过JDK 动态代理的方式，将JDBC 操作通过指定的日志框架打印出来。这个功能通常在
 * 开发阶段使用，它可以输出SQL 语句、用户传入的绑定参数、SQL 语句影响行数等等信息
 *
 *
...
public abstract class BaseJdbcLogger {
  ...
  /**
   * 记录了PreparedStatement . set ＊（）方法设置的key 值
   */
  ...
  protected Log statementLog;  // 用于输出日志的log 对象
  protected int queryStack; //  用于记录sql 的层数，格式化输出sql
